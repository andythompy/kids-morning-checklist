...
| Calendar proxy URL | `CONFIG.calendar.endpoint` in `index.html` | No (it's a public URL — keep it server-side gated) |
...
2. **Calendar (optional)** — follow [`CALENDAR_SETUP.md`](./CALENDAR_SETUP.md) to deploy the Apps Script proxy and paste its URL into `CONFIG.calendar.endpoint`. Until you do this, the calendar column shows *"Calendar not set up yet."* and the rest of the app works normally.
